############ BOOTSTRAP ARTIFACT GENERATOR — STRESS TEST ######################
#                                                                              #
# Same structure as Experiment_Bootstrap.py but uses stress_test_params        #
# (empirically derived from GS, intentionally aggressive/overfitted config)   #
# instead of the saved grid-search config.                                     #
#                                                                              #
# Saves artifacts to:                                                          #
#   PERFORMANCE_RESULTS_PAPER / Bootstrap_StressTest / {model} /              #
#     Distribution_{target}.csv   — 1000 bootstrap resamples × metrics        #
#     PointEstimates_{target}.csv — metrics on full test set                   #
#                                                                              #
# Run Experiment_Bootstrap_CI.py (pointing at Bootstrap_StressTest) to get    #
...
from config import ExperimentConfig, stress_test_params
...
if __name__ == "__main__":
    ...
    # =========================================================
    # Train / val split (val kept for consistency; not used in fit)
    # =========================================================
    ...
    # =========================================================
    # Build models using stress_test_params
    # =========================================================
    cfg = ExperimentConfig(random_state=RANDOM_STATE, model_params=stress_test_params, verbose=-1)
    print(f"Model config: stress_test_params")
    ...
    print("\nDone. Run Experiment_Bootstrap_CI.py (with Bootstrap_StressTest path) to compute CIs.")
