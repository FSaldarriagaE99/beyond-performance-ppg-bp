...
class BmExctator:
    ...
    def map_func(self):
        ...
        my_funcs = {#PPG signal
                    ...
                    "L-H ratio": "self.getRatioLH()",
                    "ShannonEntropy": "self.getShannon()",
                    ...
                    "TWRRF25": "self.getRatioSW_DW(25)",
                    "TWRRF50": "self.getRatioSW_DW(50)",
                    ...
                    "Av-Au ratio": "self.get_ratio_Av_Au()",
                    "Aw/Au": "self.get_ratio_Aw_Au()",
                    "Ab-Aa ratio": "self.get_ratio_Ab_Aa()",
                    "Ac-Aa ratio": "self.get_ratio_Ac_Aa()",
                    "Ad-Aa ratio": "self.get_ratio_Ad_Aa()",
                    "Ae/Aa": "self.get_ratio_Ae_Aa()",
                    "Af/Aa": "self.get_ratio_Af_Aa()",
                    "Ap2-Ap1 ratio": "self.get_ratio_Ap2_Ap1()",
...
# Tpp
    ...
    def getKurtosis(self):
        k = scipy.stats.kurtosis(self.segment_ppg)
        return k
    
    def getSkewness(self):
        sk = scipy.stats.skew(self.segment_ppg)
        return sk
    
    def getRatioLH(self):
        h = (self.peak_value - self.onsets_values[0])/self.peak_value #Height, normalized since some signals has bigger amplitudes
        l = self.onsets_times[1] - self.onsets_times[0] #Lenght
        ratio = h/l
        return ratio
    
    def getShannon(self):
        pk = self.segment_ppg
        pk = pk - np.min(pk) + 1e-12  # Now all values > 0
        pk = pk / np.sum(pk) #Normalize
        shent = scipy.stats.entropy(pk=pk, base=2)
        return shent
